...
module.exports = {
  ...
  async execute(message, args, client) {
    ...
    try {
      ...
      // Docker formatting
      const runningContainers = docker.filter(
        (c) => c.state === "running"
      ).length;
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          ...
          {
            name: "⏱️ Uptime",
            value: `${(uptime / 3600).toFixed(1)} hrs`,
            inline: true,
          },
          { name: "💻 CPU", value: `${cpu.avg} GHz`, inline: true },
          {
            name: "🧠 RAM",
            value: `${(mem.active / 1024 / 1024 / 1024).toFixed(1)}/${(
              mem.total /
              1024 /
              1024 /
              1024
          ...
          {
            name: "💾 Disk",
            value: `${(disk[0].used / 1024 / 1024 / 1024).toFixed(1)} GB used`,
            inline: true,
          },
          {
            name: "🐳 Docker",
            value: `${runningContainers} Running`,
            inline: true,
          }
        )
